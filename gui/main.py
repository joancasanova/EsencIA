...
async def search_huggingface_models(query: str, limit: int = 8) -> List[Dict]:
    """Busca modelos en HuggingFace Hub API."""
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            # Filtrar por modelos de generación de texto
            ...
            if response.status_code == 200:
                ...
                for m in models:
                    ...
                    results.append({
                        'value': model_id,
                        'label': f"{model_id} ({dl_str} downloads)"
                    })
...
def home_page(tabs=None):
    ...
    with ui.column().classes('w-full max-w-4xl mx-auto gap-8 p-6'):
        ...
        # Recursos del sistema - diseño compacto con barras
...
def pipeline_page():
    ...
    def update_counters():
        ...
        if steps_counter_label:
            steps_counter_label.set_text(f'{len(local_state["steps"])} pasos')
    ...
    def render_steps_list():
        ...
        # Botones para añadir pasos (secundarios)
        with ui.row().classes('w-full justify-center gap-1 mt-3'):
            ui.button(icon='auto_awesome', on_click=lambda: add_step('generate')).props('flat round size=sm').classes('text-indigo-400').tooltip('Añadir Generate')
            ui.button(icon='find_in_page', on_click=lambda: add_step('parse')).props('flat round size=sm').classes('text-amber-400').tooltip('Añadir Parse')
            ui.button(icon='verified', on_click=lambda: add_step('verify')).props('flat round size=sm').classes('text-emerald-400').tooltip('Añadir Verify')
    ...
    def render_data_section():
        ...
        with ui.row().classes('gap-2 mt-3 flex-wrap'):
            ui.button('+ Añadir entrada', icon='add', on_click=add_data_entry).props('flat size=sm')
            ui.upload(on_upload=on_data_upload, auto_upload=True).props('flat accept=.json').classes('max-w-36')
            ui.button('Cargar ejemplo', icon='folder', on_click=load_example_data).props('flat size=sm')
    ...
    with ui.column().classes('w-full max-w-6xl mx-auto gap-4 p-6'):
        ...
        # Header: Título + Selector de Modelo
        ...
        with ui.card().classes('w-full bg-slate-800/30 border border-purple-500/20 p-3'):
            with ui.row().classes('w-full justify-between items-center mb-2'):
                with ui.row().classes('items-center gap-2'):
                    ui.icon('input', size='sm').classes('text-purple-400')
                    ui.label('Datos de Entrada').classes('text-lg font-semibold text-purple-300')
                entries_counter_label = ui.label(f'{len(local_state["data_entries"])} entradas').classes('text-sm text-slate-500')
